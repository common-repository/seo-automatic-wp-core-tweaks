...
function sc_googletools() {
	include('google-tools.php');
}
...
function core_menu() {
	...
	if ($menu_added) {
		...
		add_menu_page('Core Tweaks WP by Search Commander, Inc.', 'Core Tweaks WP', 'activate_plugins', 'seo-automatic-options', 'sc_index',plugins_url() . '/seo-automatic-wp-core-tweaks/images/favicon.png');
		add_submenu_page('seo-automatic-options', 'Main', 'Main', 'activate_plugins', 'seo-automatic-options', 'sc_index');
	}
	//add_submenu_page('seo-automatic-options', 'Tab Test', 'Tab Test', 'activate_plugins', dirname(__FILE__) . '/google-tools.php', 'sc_googletools');
...
if (!get_option('seocoretweaks_ver')) {
	...
	if (!get_option('seoauto_core_user_profile_html')) { update_option('seoauto_core_user_profile_html', 'on'); }
